...
`index.html` (home), `cartas-contempladas.html` (estoque de cartas do parceiro **LD Cred** — usa o **mesmo navbar do site** (sticky + menu mobile; carrega jQuery/rd-navbar/`script.min.js` e `js/cartas.js`, que lê `data/cartas.json`): filtros por banco/código, segmento, administradora, faixa de crédito e valor desejado; estoque em **tabela no desktop** (12 colunas, ordenação por clique no cabeçalho, seleção de cotas com checkbox, paginação de 24 e **modal "Resultado da Junção"**) e **cartões no mobile** (com alternador de visão no desktop); estados de carregando/vazio/erro) e `politica-de-privacidade.html` (LGPD, página interna enxuta — header/footer simplificados, sem JS de template). As páginas `about-us.html`, `contacts.html` e `typography.html` foram **removidas do repositório** (conteúdo morto do template, sem links; recuperáveis via git history). Todo o contato já está no footer da home.
