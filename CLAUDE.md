...
| **Seção "Marpe em destaque"** | `#destaques` — **primeira seção da home (capa)** desde 2026-08: carrossel Swiper 3 (bundle legacy local, **API de params planos**: `autoplay: 6500`, `pagination: '.sel'`, `nextButton/prevButton` — não usar API de objetos do Swiper 5+) com 15 artes da campanha do cliente (`images/destaques/*.webp`, originais em `imagens_cliente_contexto/`, gitignored), cada slide com CTA WhatsApp pré-preenchido. Fundo escuro fundindo com a navbar + `padding-top: 84px` (compensa o menu `position:absolute` ~59px). Init inline no fim do body; CSS inline (`.destaque-card`, `.destaques-*`). Setas só em desktop; swipe nativo no mobile com hint `.destaques-hint`. Clones do loop recebem `aria-hidden` + `tabindex=-1` via script inline. |
...
| Banner "Melhores Ofertas" | Seção com fundo `images/banner9.webp`, botões "Cote agora pelo WhatsApp" e "Ver todos os serviços". |
| Footer | 2 colunas: "Sobre nós" (col-lg-7) + "Contato" com WhatsApp, **atendimento (seg–sex 8h30–18h)**, endereço e redes sociais (col-lg-5). Rodapé inferior com `flex-wrap` e `gap:12px`. |
...
| Coesão de cor | Azul único `#0d6efd` (overrides inline do template `#007bff`/`#4854ed` em `.button-primary`, `.button-primary-outline`, `.ui-to-top`). Descrições dos cards escurecidas para `#5f666d` (AA reforçado). |
